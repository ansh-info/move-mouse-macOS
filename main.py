...
def user_activity_listener(stop_event: threading.Event, user_event: threading.Event):
    pid = os.getpid()
    ...
    def _callback(proxy, event_type, event, _refcon):
        ...
        if source_pid != pid:
            logging.info("Detected user input from PID %s; stopping movement.", source_pid)
            user_event.set()
        return event
...
def move_cursor_randomly(args: argparse.Namespace) -> None:
    ...
    try:
        while not user_activity.is_set():
            ...
            target_x, target_y = clamp_position(current_x + dx, current_y + dy, width, height)

            logging.info(
                "Moving cursor from (%.0f, %.0f) to (%.0f, %.0f).", current_x, current_y, target_x, target_y
            )
